function varargout = coatingPermitivityProfile(varargin)
% COATINGPERMITIVITYPROFILE MATLAB code for coatingPermitivityProfile.fig
%      COATINGPERMITIVITYPROFILE, by itself, creates a new COATINGPERMITIVITYPROFILE or raises the existing
%      singleton*.
%
%      H = COATINGPERMITIVITYPROFILE returns the handle to a new COATINGPERMITIVITYPROFILE or the handle to
%      the existing singleton*.
%
%      COATINGPERMITIVITYPROFILE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in COATINGPERMITIVITYPROFILE.M with the given input arguments.
%
%      COATINGPERMITIVITYPROFILE('Property','Value',...) creates a new COATINGPERMITIVITYPROFILE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before coatingPermitivityProfile_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to coatingPermitivityProfile_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help coatingPermitivityProfile

% Last Modified by GUIDE v2.5 26-Oct-2013 20:25:06

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @coatingPermitivityProfile_OpeningFcn, ...
                   'gui_OutputFcn',  @coatingPermitivityProfile_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before coatingPermitivityProfile is made visible.
function coatingPermitivityProfile_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to coatingPermitivityProfile (see VARARGIN)

% Choose default command line output for coatingPermitivityProfile
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes coatingPermitivityProfile wait for user response (see UIRESUME)
% uiwait(handles.coatingPermitivityProfile);
...
function varargout = coatingPermitivityProfile_OutputFcn(hObject, eventdata, handles) 
...
currentOpticalSystem1.SurfaceArray(surfIndex).Coating.plotCoatingPermitivityProfile(handles.axesRefIndexProfile);
