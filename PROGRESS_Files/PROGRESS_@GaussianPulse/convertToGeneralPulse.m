...
HarmonicFieldArray(nWavSampling) = HarmonicField;
for ww = 1:nWavSampling
    Ex = spatialAmplitude.*exp(1i*spatialPhase)*spectralAmplitude(ww)*exp(1i*spectralPhase(ww));
    Ey = 0*Ex;
    HarmonicFieldArray(ww) = HarmonicField(Ex,Ey,dx,dy,dir,waveLenVector(ww));
end
...
generalPulse = GeneralPulse(HarmonicFieldArray,direction);
