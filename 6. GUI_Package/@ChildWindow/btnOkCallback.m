function btnOkCallback(childWindow,parentWindow)
    ...
    switch lower(handles.Name)
            ...
            switch lower(handles.Name)
                case lower('amplitudeTransmissionMap')
                    plotAmplitudeTransmissionMap...
                        ...
                        sampleGridSize,handles.panelMainTab(2));
                case lower('phaseMap')
                    plotPhaseMap...
                        (optSystem,surfIndex,wavLen,fieldPointXY,...
                        sampleGridSize,handles.panelMainTab(2));
                ...
                case lower('polarizationEllipseMap')
                    ...
                    plotPupilPolarizationEllipseMap...
                        (optSystem,surfIndex,wavLen,fieldPointXY,...
                        sampleGridSize,jonesVector,coordinate,handles.panelMainTab(2)) ;
                case lower('wavefrontAtExitPupil')
                    ...
                    plotWavefrontAtExitPupil(optSystem,wavLen,...
                        ...
                        handles.panelMainTab(2),handles.textHandle) ;
                case lower('FFTPSF')
        ...
        case lower(rayTracingAnalysis)
            ...
            rayTracerResult = rayTracer(optSystem,newRay,considerPolarization,considerSurfAperture,recordIntermediateResults);
            ...
            switch lower(handles.Name)
                case lower('scalarRayTrace')
                    if isempty(rayTracerResult)||...
                            ~strcmpi(class(rayTracerResult),'RayTraceResult')
                        ...
                        result = [[1:nSurf]',...
                            ...
                            [rayTracerResult.IncidenceAngle]'*180/pi,...
                            [rayTracerResult.PathLength]'];
                        ...
                        for kk = 1:1:nSurf
                            ...
                            pathLen = rayTracerResult(kk).PathLength;
                            ...
                            rayTraceResultSeqText = char(rayTraceResultSeqText,...
                                ...
                                ['Path Length                       : ', num2str(pathLen,'%+12.4e')]);
                        end
                        if showInTabular
        ...
        case lower(others)
            switch lower(handles.Name)
                ...
                case lower('transverseRayAberration')
                    ...
                    % handles.panelMainTab(2) is passed instead of the axes
                    % handle.
                ...
                case lower('KostenbauderMatrix')
                    ...
                    [rayPosition,rayDirection] = computeRayParametersFromNormalizedCoordiantes...
                        (optSystem,Hx,Hy,Px,Py);
                    pilotRay = ScalarRay(rayPosition,rayDirection,wavLenInM) ;
