function [ validSystem,message ] = validateOpticalSystemNew(parentWindow)
...
%     tempImageHeight = get(aodHandles.radioImageHeight,'Value');
    ...
    if abs(objThickness) > 10^10
        if strcmpi(tempFieldType,'ObjectHeight')
            message = ('For objects at infinity, object height can not be used as field please correct.');
            valid = 0;
        elseif strcmpi(tempSystemApertureType,'OBNA')
