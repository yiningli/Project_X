classdef Glass
    ...
    properties
        % standard data
        Name % (String) - Name of the glass by which they are identified.
        Type % (String) - The glass type. It is the same as the glass definition function name.
        Comment % (String) - Glass coment
        Parameters % (Struct) - Structure containg the parameters used for different types of glass.
        InternalTransmittance % (Matrix Nx3) - Matrix of Lambda,Transmission,Thickness
        ThermalData % (Vector 10x1) - [D0,D1,D2,E0,E1,Ltk,Temp,TCE70, TCE300,Ignore_Thermal_Exp];
        WavelengthRange % (Vector 2x1) - [Min_Lambda Max_Lambda]
        ResistanceData % (Vector 5x1) - [ClimateResistance,StainResistance,AcidResistance,AlkaliResistance,PhosphateResistance]
        OtherData % (Vector 8x1) - [RelativeCost,Density, MeltingFrequency, Status, ExcludeFromSubstitution, MIL_Number,Nd,Vd,dPgF ]
    ...
    methods(Static)
        function newObj = InputGUI()
            newObj = ObjectInputDialog(MyHandle(Glass()));
        end
    end
end
