function myNonDummySurfaceArray = getNonDummySurfaceArray(currentComponent)
    % getNonDummySurfaceArray: Compute surface parameters of the currentComponent
    ...
    % <<<<<<<<<<<<<<<<<<<<< Main Code Section >>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    mySurfaceArray = getSurfaceArray(currentComponent);
