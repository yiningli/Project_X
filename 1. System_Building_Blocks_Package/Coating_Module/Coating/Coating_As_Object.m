classdef Coating
    ...
    % <<<<<<<<<<<<<<<<<<<<< Main Code Section >>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    properties
        Name % (String) - Name of the coating by which they are identified.
        Type % (String) - The coating type. It is the same as the coating definition function name.
        Parameters % (Struct) - Structure containg the parameters used for different types of coating.
    ...
    methods(Static)
        function newObj = InputGUI()
            newObj = ObjectInputDialog(MyHandle(Coating()));
        end
    end
end
